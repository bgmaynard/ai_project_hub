...
def main():
    ...
    for p in files:
        ...
        missing_local = [
            m
            for m in imps
            if m not in ("__future__",) and not check_import_available(m)
        ]
        if missing_local:
    ...
    lines.append(
        ...
        if "error" in report["pytest_result"]
        else f"- returncode: {report['pytest_result'].get('returncode')}\n```\n{report['pytest_result'].get('output','')}\n```"
    )
    lines.append("")
