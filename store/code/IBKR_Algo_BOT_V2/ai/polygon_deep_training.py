...
class PolygonDataFetcher:
    ...
    def fetch_daily_bars(
        self, symbol: str, years: int = 10, use_cache: bool = True
        ...
        try:
            ...
            df = df[
                ["date", "open", "high", "low", "close", "volume", "vwap", "trades"]
            ]
    ...
    def fetch_minute_bars(
        ...
        try:
            ...
            df = df[
                ["datetime", "open", "high", "low", "close", "volume", "vwap", "trades"]
            ]
...
class FeatureEngineering:
    ...
    def compute_daily_features(df: pd.DataFrame) -> pd.DataFrame:
        ...
        data["bb_pct"] = (data["close"] - data["bb_lower"]) / (
        ...
        data["gap_filled"] = (
            (data["low"] <= data["close"].shift()) & (data["gap"] > 0)
        ...
        data["hh_streak"] = (
            data["hh"].groupby((data["hh"] != data["hh"].shift()).cumsum()).cumsum()
        )
        data["ll_streak"] = (
            data["ll"].groupby((data["ll"] != data["ll"].shift()).cumsum()).cumsum()
        )
...
class ModelTrainer:
    ...
    def train_lightgbm_daily(self, df: pd.DataFrame) -> Dict:
        ...
        feature_cols = [
            c for c in df.columns if c not in exclude_cols and not df[c].isna().all()
        ]
        ...
        logger.info(
            f"Training LightGBM: {len(X)} samples, {len(feature_cols)} features"
        )
        ...
        with open(model_path, "wb") as f:
            pickle.dump(
                {"model": best_model, "features": feature_cols, "params": params}, f
            )

        # Feature importance
        importance = pd.DataFrame(
            {"feature": feature_cols, "importance": best_model.feature_importance()}
    ...
    def train_scalp_model(self, df: pd.DataFrame) -> Dict:
        ...
        logger.info(
            f"Training Scalp Model: {len(X)} spike events, {len(feature_cols)} features"
        )
...
def run_deep_training():
    ...
    scalp_symbols = (
        TRAINING_UNIVERSE["small_momentum"] + TRAINING_UNIVERSE["recent_movers"][:5]
    )
    ...
    if minute_dfs:
        ...
        logger.info(
            f"  Minute data: {len(combined_minute)} bars from {len(minute_dfs)} symbols"
        )
        ...
        if scalp_results:
            ...
            logger.info(
                f"  Spike continuation rate: {scalp_results.get('spike_continuation_rate', 0):.2%}"
            )
    ...
    if "lightgbm_daily" in results["models"]:
        logger.info(
            f"Daily Model Accuracy: {results['models']['lightgbm_daily']['avg_accuracy']:.2%}"
        )
    if "scalp_model" in results["models"]:
        logger.info(
            f"Scalp Model AUC: {results['models']['scalp_model'].get('val_auc', 0):.4f}"
        )
