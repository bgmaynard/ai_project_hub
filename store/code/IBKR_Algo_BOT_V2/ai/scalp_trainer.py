...
SCALP_SYMBOLS = [
    ...
    # Volatile small caps
    'SOFI', 'HOOD', 'AFRM', 'UPST', 'FUBO',
    # Biotech volatility
    'NVAX', 'MRNA', 'SAVA', 'OCGN', 'INO',
    # EV plays
...
class PolygonMinuteData:
    ...
    def get_minute_bars(
        ...
        try:
            ...
            df = df[['timestamp', 'open', 'high', 'low', 'close', 'volume', 'vwap', 'trades']]
...
class ScalpFeatureEngine:
    ...
    def compute_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        ) & (data['vol_surge'] > 1.5)  # With moderate volume surge

        # FORWARD TARGET: Does spike continue?
        # Look at return over next N minutes
        data['forward_return'] = data['close'].shift(-self.forward_minutes) / data['close'] - 1
        ...
        # Feature columns
        feature_cols = [
            'return_1m', 'return_2m', 'return_5m',
            'vol_surge', 'velocity_1m', 'velocity_2m',
            'range_pct', 'vwap_dev',
            'trades_surge',
            'is_premarket', 'is_open', 'is_power_hour',
            'prev_5m_trend',
            'hour', 'minute'
        ]
...
class ScalpModelTrainer:
    ...
    def collect_training_data(
        ...
        logger.info(f"Collecting {days} days of minute data for {len(symbols)} symbols...")
        ...
        for i, symbol in enumerate(symbols):
            try:
                ...
                feature_cols = [c for c in features.columns if c not in ['target', 'target_continues', 'target_strong', 'forward_return']]

                all_features.append(features[feature_cols])
                all_labels.append(features['target'])  # Using 1% continuation as target

                logger.info(f"  [{i+1}/{len(symbols)}] {symbol}: {len(features)} spike events")
    ...
    def save(self, model_path: str = "ai/scalp_model.pkl", meta_path: str = "ai/scalp_model_meta.json"):
        """Save model"""
