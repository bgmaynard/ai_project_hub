...
def main():
    ...
    test_configs = [
        {
            "name": "Current (7%/5x)",
            "min_spike_percent": 7.0,
            "min_volume_surge": 5.0,
            "stop_loss_percent": 3.0,
            "trailing_stop_percent": 2.0,
        },
        {
            "name": "Lower spike (5%/5x)",
        ...
        {
            "name": "Lower spike (6%/5x)",
            "min_spike_percent": 6.0,
            "min_volume_surge": 5.0,
            "stop_loss_percent": 3.0,
            "trailing_stop_percent": 2.0,
        },
        {
            "name": "Lower volume (7%/3x)",
            "min_spike_percent": 7.0,
        ...
        },
        {
            "name": "Balanced (6%/4x)",
            "min_spike_percent": 6.0,
            "min_volume_surge": 4.0,
            "stop_loss_percent": 3.0,
            "trailing_stop_percent": 2.0,
        },
        {
            "name": "Aggressive (5%/3x)",
            "min_spike_percent": 5.0,
            "min_volume_surge": 3.0,
            "stop_loss_percent": 3.0,
            "trailing_stop_percent": 2.0,
        ...
        {
            "name": "Conservative (8%/6x)",
            "min_spike_percent": 8.0,
            "min_volume_surge": 6.0,
            "stop_loss_percent": 3.0,
            "trailing_stop_percent": 2.0,
        },
    ]
    ...
    print("=" * 70)
    print(
        f"{'Config':<25} {'Trades':>7} {'Wins':>6} {'Win%':>7} {'P/L':>10} {'Return':>8}"
    )
    print("-" * 70)
    ...
    for config in test_configs:
        ...
        print(
            f"{name:<25} {trades:>7} {wins:>6} {win_rate:>6.1f}% ${pnl:>8.2f} {ret:>7.2f}%"
        )
    ...
    print(
        f"  P/L: ${best_pnl['pnl']}, Win Rate: {best_pnl['win_rate']}%, Trades: {best_pnl['trades']}"
    )
    ...
    if filtered:
        ...
        print(
            f"  Win Rate: {best_wr['win_rate']}%, P/L: ${best_wr['pnl']}, Trades: {best_wr['trades']}"
        )
    ...
    if trades_filtered:
        ...
        print(
            f"  P/L/Trade: ${best_risk['pnl_per_trade']:.2f}, Total P/L: ${best_risk['pnl']}, Trades: {best_risk['trades']}"
        )
    ...
    for k, v in best["params"].items():
        print(f"    {k}: {v}")
    print(
        f"  Expected: {best['trades']} trades, {best['win_rate']}% win rate, ${best['pnl']} P/L"
    )

    # Save results
    with open(
        "C:/ai_project_hub/store/code/IBKR_Algo_BOT_V2/ai/backtest_results.json", "w"
    ) as f:
