...
class TapeAnalyzer:
    ...
    def analyze(self, symbol: str, lookback_seconds: int = 60) -> TapeAnalysis:
        ...
        """
        analysis = TapeAnalysis(symbol=symbol, timestamp=datetime.now())
        ...
        recent_trades = [t for t in self.tape_history[symbol] if t.timestamp > cutoff]
    ...
    def _detect_first_green(
        self, trades: List[TapeEntry], red_streak_min: int = 5
    ) -> bool:
        """
    ...
    def detect_irrational_flush(
        self, symbol: str, lookback_seconds: int = 30, min_drop_pct: float = 3.0
    ) -> Dict:
        """
        ...
        high_price = max(
            t.price for t in recent[: len(recent) // 2]
        )  # High in first half
