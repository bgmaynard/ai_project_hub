    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "time_in_state_seconds": (
                datetime.now() - self.entered_state_at
            ).total_seconds(),
...
class MomentumStateMachine:
    ...
    def configure(
        ...
        igniting_score: int = None,
        gated_score: int = None,
    ):
        ...
        logger.info(
            f"FSM thresholds configured: CANDIDATE={self.candidate_score}, IGNITING={self.igniting_score}, GATED={self.gated_score}"
        )
    ...
    def _is_valid_transition(
        self, from_state: MomentumState, to_state: MomentumState
    ) -> bool:
    ...
    def update_momentum(
        ...
        if vetoed:
            ...
            if current_state in [MomentumState.IGNITING, MomentumState.GATED]:
                self.transition(
                    symbol,
                    MomentumState.CANDIDATE,
                    TransitionReason.VETO_TRIGGERED,
                    details,
                )
        ...
        if current_state == MomentumState.IDLE and target_state != MomentumState.IDLE:
            self.transition(
                symbol,
                MomentumState.CANDIDATE,
                TransitionReason.MOMENTUM_SCORE,
                details,
            )
        ...
        if current_state == MomentumState.CANDIDATE:
            if target_state == MomentumState.IDLE:
                self.transition(
                    symbol, MomentumState.IDLE, TransitionReason.MOMENTUM_LOST, details
                )
        ...
        if current_state == MomentumState.IGNITING:
            if target_state == MomentumState.GATED:
                ...
                self.transition(
                    symbol,
                    MomentumState.GATED,
                    TransitionReason.GATING_REQUESTED,
                    details,
                )
                ...
                self.transition(
                    symbol,
                    MomentumState.CANDIDATE,
                    TransitionReason.MOMENTUM_LOST,
                    details,
                )
                return MomentumState.CANDIDATE
        ...
        if self.gating_manager:
            try:
                approved, exec_request, reason = self.gating_manager.gate_trade_attempt(
                    symbol=symbol, trigger_type="momentum_ignition", quote=quote or {}
                )
                ...
                if approved:
                    ...
                    self.transition(
                        symbol,
                        MomentumState.IGNITING,
                        TransitionReason.GATING_REJECTED,
                        {"reason": reason},
                    )
            ...
            logger.warning(
                f"GATING REJECTED (fail-closed): {symbol} - Gating manager unavailable"
            )
            self._fire_callback("on_gating_rejected", symbol, state)
            self.transition(
                symbol,
            ...
            return {
                "approved": False,
                "reason": "Gating manager unavailable - fail-closed",
            }
    ...
    def enter_position(
        ...
        if not state or state.state not in [
            ...
            logger.warning(
                f"Cannot enter position for {symbol} - not in GATED/IGNITING state"
            )
            return False
    ...
    def start_monitoring(self, symbol: str) -> bool:
        ...
        return self.transition(
            symbol, MomentumState.MONITORING, TransitionReason.POSITION_ENTERED
        )
    ...
    def update_position(self, symbol: str, current_price: float):
        ...
        if state.state == MomentumState.MONITORING:
            state.time_in_monitoring = (
                datetime.now() - state.entered_state_at
            ).total_seconds()

    def signal_exit(
        self, symbol: str, exit_signal: str, confidence: float = 0, pnl_pct: float = 0
    ) -> bool:
        """
        ...
        if not state or state.state not in [
            ...
            logger.warning(
                f"Cannot signal exit for {symbol} - not in position/monitoring"
            )
            return False
        ...
        return self.transition(
            ...
            MomentumState.EXITING,
            reason,
            {"exit_signal": exit_signal, "confidence": confidence, "pnl_pct": pnl_pct},
        )
    ...
    def check_timeouts(self) -> List[str]:
        ...
        for symbol, state in list(self._states.items()):
            ...
            if (
                state.state == MomentumState.CANDIDATE
                and time_in_state > self.CANDIDATE_TIMEOUT
            ):
                self.transition(symbol, MomentumState.IDLE, TransitionReason.TIMEOUT)
                reset_symbols.append(symbol)

            elif (
                state.state == MomentumState.IGNITING
                and time_in_state > self.IGNITING_TIMEOUT
            ):
                self.transition(
                    symbol, MomentumState.CANDIDATE, TransitionReason.TIMEOUT
                )
                reset_symbols.append(symbol)

            elif (
                state.state == MomentumState.GATED
                and time_in_state > self.GATED_TIMEOUT
            ):
                # Gating window expired without decision
                self.transition(
                    symbol, MomentumState.IGNITING, TransitionReason.TIMEOUT
                )
                reset_symbols.append(symbol)

            elif (
                state.state == MomentumState.IN_POSITION
                and time_in_state > self.IN_POSITION_TIMEOUT
            ):
                # Auto-transition to MONITORING
                self.start_monitoring(symbol)

            elif (
                state.state == MomentumState.EXITING
                and time_in_state > self.EXITING_TIMEOUT
            ):
                ...
                and time_in_state > self.COOLDOWN_DURATION
            ):
                self.transition(
                    symbol, MomentumState.IDLE, TransitionReason.COOLDOWN_EXPIRED
                )
                reset_symbols.append(symbol)
    ...
    def exit_position(
        self, symbol: str, reason: TransitionReason, pnl_pct: float = 0
    ) -> bool:
...
if __name__ == "__main__":
    ...
    for t in sm.get_transition_log():
        print(
            f"  {t['from_state']} -> {t['to_state']} ({t['reason']}) [owner: {t['owner']}]"
        )
