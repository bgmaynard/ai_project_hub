...
from ai.gap_grader import GapGrade, GapGrader
...
def get_historical_data(symbol: str, trade_date: str) -> Dict:
    ...
    try:
        ...
        if len(hist) >= 3:
            prior_change = (
                (hist.iloc[-2]["Close"] - hist.iloc[-3]["Close"])
                / hist.iloc[-3]["Close"]
            ) * 100
...
def simulate_gap_grade(trade: Dict, hist_data: Dict) -> Tuple[str, int]:
    ...
    gap_pct = (
        ((entry_price - prior_close) / prior_close) * 100 if prior_close > 0 else 5.0
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--min-grade", "-g", default="C", help="Minimum grade to accept (A, B, C, D, F)"
    )
