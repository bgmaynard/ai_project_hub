...
class ExitImperativeEngine:
    ...
    def register_position(
        ...
        self._monitored_positions[symbol] = PositionContext(
            ...
            or entry_price * (1 + self.config["profit_target_pct"]),
            high_since_entry=entry_price,
        )
        self._decay_counts[symbol] = 0
        logger.info(
            f"Registered position for exit monitoring: {symbol} @ {entry_price}"
        )
    ...
    def check_exit(
        ...
        if (
            self.config["regime_shift_exit"]
            and regime in self.config["invalid_regimes"]
        ):
            results.append(
                ExitImperativeResult(
                    should_exit=True,
        ...
        if (
            self.config["volatility_exit_enabled"]
            and volatility > self.config["max_volatility"]
        ):
            results.append(
                ExitImperativeResult(
                    should_exit=True,
        ...
        if momentum_state == "DECAY":
            ...
            if self._decay_counts[symbol] >= self.config[
                "decay_consecutive_checks"
            ] and (not self.config["decay_vwap_required"] or not above_vwap):
                results.append(
                    ExitImperativeResult(
                        should_exit=True,
    ...
    def get_exit_stats(self) -> Dict:
        ...
        return {
            ...
            "avg_time_in_position": round(
                sum(e.time_in_position_seconds for e in self._exit_log)
                / len(self._exit_log),
                1,
            ),
        }
...
if __name__ == "__main__":
    ...
    engine.register_position(
        symbol, entry_price=5.00, shares=100, stop_price=4.85, target_price=5.15
    )

    print("\n1. Initial position check...")
    engine.update_position(symbol, 5.02)
    result = engine.check_exit(
        symbol, momentum_state="CONFIRMED", regime="TRENDING_UP", volatility=0.02
    )
    ...
    result = engine.check_exit(
        symbol, momentum_state="DECAY", regime="TRENDING_UP", volatility=0.02
    )
    print(f"   Should exit: {result.should_exit}")
    ...
    result = engine.check_exit(
        symbol, momentum_state="CONFIRMED", regime="VOLATILE", volatility=0.06
    )
    print(f"   Should exit: {result.should_exit}")
    print(f"   Reason: {result.reason.value if result.reason else 'None'}")
    ...
    for i in range(3):
        ...
        result = engine.check_exit(
            symbol, momentum_state="DECAY", regime="TRENDING_UP", above_vwap=False
        )
