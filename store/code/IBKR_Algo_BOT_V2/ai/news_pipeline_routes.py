...
class NewsHeat:
    ...
    HOT_KEYWORDS = [
        ...
        'sec investigation', 'fraud', 'lawsuit filed',
        'ceo resign', 'ceo steps down', 'cfo resign',
        'bankruptcy', 'delisting', 'halt', 'halted',
        'breakthrough', 'clinical trial success', 'trial failure',
        'contract win', 'major contract', 'billion dollar'
    ]

    WARM_KEYWORDS = [
        'upgrade', 'downgrade', 'price target',
        'analyst', 'rating change',
        'product launch', 'new product', 'partnership',
        'guidance', 'outlook', 'forecast update',
        'expansion', 'new market', 'strategic',
        'insider buying', 'insider selling', 'buyback'
    ]
...
async def get_alerts(limit: int = 20):
    ...
    try:
        ...
        return {
            "success": True,
            "alerts": pipeline.get_alerts(limit)
        }
    except Exception as e:
...
async def get_halt_history(limit: int = 20):
    ...
    try:
        ...
        return {
            "success": True,
            "history": detector.get_halt_history(limit)
        }
    except Exception as e:
...
async def get_halt_analytics_history(limit: int = 50, symbol: str = None):
    ...
    try:
        ...
        return {
            "success": True,
            "history": analytics.get_history(limit, symbol)
        }
    except Exception as e:
...
async def get_halt_strategies():
    ...
    try:
        ...
        return {
            "success": True,
            "strategies": {name: s.to_dict() for name, s in analytics.strategies.items()}
        }
    except Exception as e:
