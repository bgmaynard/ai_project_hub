...
class LossCutter:
    ...
    def __init__(self):
        ...
        self.time_threshold_minutes = (
            5  # If not profitable after 5 min = MOMENTUM FAILED
        )
        ...
        logger.info(
            "LossCutter initialized - MOMENTUM ONLY: 1% max loss, 5min stale rule, 15min max hold"
        )

    def evaluate_position(
        self, position: Dict, market_data: Dict = None
    ) -> PositionEvaluation:
        """
        ...
        pnl_pct = (
            float(position.get("unrealized_plpc", 0)) * 100
        ...
        if pnl_pct <= self.max_loss_pct:
            ...
            urgency = 5  # IMMEDIATE
            recommendation = (
                f"CLOSE IMMEDIATELY - Cut loss at {pnl_pct:.1f}% to preserve capital"
            )
        ...
        # Rule 2: Alert level - getting close to max loss
            ...
            urgency = 4
            recommendation = (
                f"STRONGLY consider closing - Don't let this hit {self.max_loss_pct}%"
            )
        ...
        # === PROFIT MANAGEMENT RULES ===
            ...
            urgency = 2
            recommendation = (
                f"Take partial profit at +{pnl_pct:.1f}% - let rest run with trail"
            )
    ...
    def get_capital_preservation_status(
        self, positions: List[Dict], account: Dict
    ) -> Dict:
        ...
        return {
            ...
            "positions_requiring_action": evaluation["summary"][
                "positions_to_cut_immediately"
            ],
