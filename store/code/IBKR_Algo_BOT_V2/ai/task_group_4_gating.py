    ...
    try:
        ...
        if r8_path.exists():
            with open(r8_path, "r") as f:
                r8_data = json.load(f)
                persistence_data = {
                    p["symbol"]: p for p in r8_data.get("predictions", [])
                }
        ...
        for candidate in candidates:
            ...
            checks = {
                ...
                "rel_vol_check": rel_vol >= MIN_REL_VOL
                or rel_vol == 0,  # 0 means no data
            }
...
def register_gating_tasks():
    ...
    manager.register_task(
        Task(
            ...
            group="4_GATING",
            inputs=[
                "report_R8_momentum_persistence.json",
                "report_R9_pullback_depth.json",
            ],
