...
def run_enhanced_strategy(
    data: pd.DataFrame, initial_cash: float, chronos_signal: dict
) -> dict:
    """
...
def main():
    ...
    for symbol in symbols:
        ...
        print(
            f"  Chronos: {chronos_signal['signal']} (prob_up={chronos_signal['prob_up']:.1%})"
        )
    ...
    if all_results:
        ...
        for r in all_results:
            print(
                f"{r['symbol']:<8} {r['chronos_signal']:<15} ${r['baseline_pnl']:>8.2f} ${r['enhanced_pnl']:>8.2f} ${r['improvement']:>+8.2f}"
            )

        print("-" * 60)
        print(
            f"{'TOTAL':<8} {'':<15} ${total_baseline:>8.2f} ${total_enhanced:>8.2f} ${total_improvement:>+8.2f}"
        )
