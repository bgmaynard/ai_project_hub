...
class NewsTradeCandidate:
    ...
    def to_dict(self) -> Dict:
        return {**asdict(self), "news_detected_at": self.news_detected_at.isoformat()}
...
class NewsAutoTrader:
        ...
        try:
            ...
            if price is not None:
                if price < self.config.min_price:
                    return (
                        False,
                        f"price ${price:.2f} < ${self.config.min_price:.2f} min",
                        stock_data,
                    )
                if price > self.config.max_price:
                    return (
                        False,
                        f"price ${price:.2f} > ${self.config.max_price:.2f} max",
                        stock_data,
                    )
            else:
                logger.warning(
                    f"Could not get price for {symbol}, skipping price check"
                )
            ...
            if float_shares is not None:
                ...
                logger.warning(
                    f"Could not get float for {symbol}, skipping float check"
                )
            ...
            if avg_volume is not None:
                if avg_volume < self.config.min_avg_volume:
                    return (
                        False,
                        f"avg volume {avg_volume:,} < {self.config.min_avg_volume:,} min",
                        stock_data,
                    )
            ...
            # All checks passed!
            logger.info(
                f"Stock {symbol} PASSED screening: price=${price}, float={float_shares}, vol={avg_volume}"
            )
            return True, None, stock_data
    ...
    async def start(self):
        ...
        logger.info(
            f"NewsAutoTrader STARTED - {'PAPER' if self.config.paper_mode else 'LIVE'} mode"
        )
        logger.info(
            f"Monitoring {len(symbols)} symbols, config: {self.config.to_dict()}"
        )
    ...
    def _on_sell_signal(self, alert):
        ...
        logger.info(
            f"SELL signal ignored (no shorting): {alert.symbols} - {alert.headline[:50]}"
        )
    ...
    async def _process_candidate(self, symbol: str, alert):
        ...
        if self.config.screen_before_add:
            passed, rejection, stock_data = await self._check_stock_requirements(
                symbol, catalyst=alert.catalyst_type
            )
            if not passed:
                ...
                logger.info(
                    f"Candidate {symbol} PASSED screening: ${price}, float={float_str}"
                )
            ...
            if chronos:
                try:
                    ...
                    if candidate.chronos_score < self.config.min_chronos_score:
                        return (
                            False,
                            f"Chronos {candidate.chronos_score:.0%} < {self.config.min_chronos_score:.0%}",
                        )
                    if candidate.chronos_direction != "bullish":
                        return (
                            False,
                            f"Chronos direction: {candidate.chronos_direction}",
                        )
                except Exception as e:
            ...
            if qlib:
                try:
                    ...
                    if score < self.config.min_qlib_score:
                        return (
                            False,
                            f"Qlib {score:.0%} < {self.config.min_qlib_score:.0%}",
                        )
                except Exception as e:
                ...
                try:
                    ...
                    if signal:
                        ...
                        if signal.buy_pressure < self.config.min_buy_pressure:
                            return (
                                False,
                                f"Buy pressure {signal.buy_pressure:.0%} < {self.config.min_buy_pressure:.0%}",
                            )
    ...
    async def _add_to_watchlist(
        self, symbol: str, headline: str = "", catalyst: str = ""
    ):
    ...
    async def _execute_trade(self, candidate: NewsTradeCandidate):
        ...
        try:
            ...
            if self.config.paper_mode:
                ...
                candidate.trade_result = (
                    f"paper_trade_triggered (gated: {gating_token})"
                )
                logger.warning(
                    f"PAPER TRADE (GATED): Would buy {symbol} - {candidate.news_headline[:50]}"
                )
            else:
                ...
                logger.warning(
                    f"LIVE TRADE (GATED): Queueing {symbol} for scalper entry"
                )
        ...
        if expired:
            try:
                ...
                if watchlist:
                    ...
                    logger.info(
                        f"Cleaned up {len(expired)} expired symbols from watchlist"
                    )
            except Exception as e:
