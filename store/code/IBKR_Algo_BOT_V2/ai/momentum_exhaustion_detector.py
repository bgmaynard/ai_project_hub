...
class MomentumExhaustionDetector:
    ...
    def update_quote(self, symbol: str, bid: float, ask: float) -> Optional[ExhaustionAlert]:
    ...
    def _check_exhaustion(self, state: SymbolState, price: float, volume: int) -> Optional[ExhaustionAlert]:
    ...
    def _check_rsi_divergence(self, state: SymbolState, price: float) -> Optional[ExhaustionAlert]:
        ...
        if len(recent_highs) >= 2:
            ...
            rsi_lower = curr_high.rsi < prev_high.rsi - self.config["rsi_divergence_threshold"]

            if price_higher and rsi_lower:
                confidence = min(90, 60 + (prev_high.rsi - curr_high.rsi))
                severity = ExhaustionSeverity.HIGH if confidence > 75 else ExhaustionSeverity.MEDIUM
    ...
    def _check_volume_exhaustion(self, state: SymbolState, price: float, volume: int) -> Optional[ExhaustionAlert]:
        ...
        if len(recent_vols) >= 3:
            vol_change = (recent_vols[-1] - recent_vols[0]) / recent_vols[0] if recent_vols[0] > 0 else 0
    ...
    def _check_red_candles(self, state: SymbolState, price: float) -> Optional[ExhaustionAlert]:
    ...
    def _check_price_stalling(self, state: SymbolState, price: float) -> Optional[ExhaustionAlert]:
        ...
        if distance_from_high <= self.config["stall_range_pct"]:
            ...
            if state.failed_breakout_attempts >= self.config["stall_attempts_threshold"]:
                return self._create_alert(
                    state,
    ...
    def _check_climax_top(self, state: SymbolState, price: float, volume: int) -> Optional[ExhaustionAlert]:
        ...
        is_climax = (
            vol_ratio > 2.0 and
            wick_ratio > 0.4 and
            state.current_rsi > self.config["rsi_overbought"]
        )
    ...
    def check_exit(self, symbol: str, current_price: float) -> Optional[ExhaustionAlert]:
        """
    ...
    def get_status(self) -> Dict:
        """Get detector status"""
        exhausted = [s for s in self.symbols.values()
                     if any(a.should_exit for a in s.active_alerts)]

        return {
            "symbols_tracked": len(self.symbols),
            "symbols_with_positions": len([s for s in self.symbols.values() if s.entry_price > 0]),
