...
class WarriorSetupDetector:
    ...
    async def analyze(
        ...
        signal = WarriorSignal(symbol=symbol, timestamp=datetime.now())
        ...
        if self._halt_detector and quote:
            ...
            if halt_status.get("warning"):
                ...
                if self.config["avoid_approaching_halts"]:
                    reasons.append(
                        f"Approaching LULD band: {halt_status.get('approaching')}"
                    )
        ...
        if self._pattern_detector:
            ...
            # Find best pattern
            detected = [(name, p) for name, p in pattern_results.items() if p.detected]
            if detected:
                ...
                reasons.append(
                    f"Pattern: {best_pattern.pattern_type} ({best_pattern.confidence:.0%})"
                )
        ...
        if self._tape_analyzer:
            ...
            if signal.tape_signal != "NO_SIGNAL":
                reasons.append(
                    f"Tape: {signal.tape_signal} ({tape_signal.get('confidence', 0):.0%})"
                )
        ...
        if self._setup_classifier and stock_info:
            ...
            criteria = StockCriteria(
                ...
                rvol_over_5x=(
                    (
                        stock_info.get("volume", 0) / stock_info.get("avg_volume", 1)
                        >= 5.0
                    )
                    if stock_info.get("avg_volume", 0) > 0
                    else False
                ),
        ...
        if current_grade_order <= min_grade_order:
            if signal.setup_confidence >= self.config["min_confidence"]:
                ...
                if signal.entry_price and signal.stop_loss and signal.target_price:
                    ...
                    if risk > 0 and reward / risk >= self.config["min_risk_reward"]:
                        ...
                        reasons.append(
                            f"R:R below minimum ({reward/risk if risk > 0 else 0:.1f}:1)"
                        )
            ...
            if signal.setup_grade != "F":
                reasons.append(
                    f"Grade {signal.setup_grade} below minimum {self.config['min_setup_grade']}"
                )
        ...
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.get(
                    f"http://localhost:9100/api/worklist", timeout=5.0
                )
    ...
    def calculate_luld_bands(self, symbol: str, reference_price: float, tier: int = 2):
        ...
        if self._halt_detector:
            return self._halt_detector.calculate_luld_bands(
                symbol, reference_price, tier
            )
        return None
    ...
    def get_status(self) -> Dict:
        ...
        return {
            "pattern_detector": {
                ...
                "symbols": (
                    list(self._pattern_detector.candle_history.keys())
                    if self._pattern_detector
                    else []
                ),
            },
            "tape_analyzer": {
                ...
                "symbols": (
                    list(self._tape_analyzer.tape_history.keys())
                    if self._tape_analyzer
                    else []
                ),
            },
            "setup_classifier": {"available": self._setup_classifier is not None},
            "halt_detector": {
                "available": self._halt_detector is not None,
                "halted": (
                    list(self._halt_detector.halted_stocks.keys())
                    if self._halt_detector
                    else []
                ),
                "luld_tracking": (
                    list(self._halt_detector.luld_bands.keys())
                    if self._halt_detector
                    else []
                ),
            },
