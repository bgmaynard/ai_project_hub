...
class AutoTrader:
    ...
    def should_trade(self, symbol: str) -> Dict[str, Any]:
        ...
        if prediction["confidence"] < self.config.min_confidence:
            result["reason"] = (
                f"Confidence too low ({prediction['confidence']:.2%} < {self.config.min_confidence:.2%})"
            )
            return result
        ...
        if prediction["prob_up"] < self.config.min_prob_up:
            result["reason"] = (
                f"Probability too low ({prediction['prob_up']:.2%} < {self.config.min_prob_up:.2%})"
            )
            return result
        ...
        if prediction["prediction"] == 1:
            ...
            result["reason"] = (
                f"BULLISH: {prediction['confidence']:.2%} confidence, {prediction['signal_detail']}"
            )
            return result
