...
class PatternCorrelator:
    ...
    async def record_mover(self, symbol: str, quote: Dict, context: Dict = None) -> MoverRecord:
        """
    ...
    def analyze_correlations(self) -> Dict:
        ...
        results['by_change_percent'] = self._analyze_factor(
            completed,
            lambda r: self._bucket_change(r.change_percent),
            "change_percent"
        )
        ...
        results['by_rel_volume'] = self._analyze_factor(
            completed,
            lambda r: self._bucket_rvol(r.relative_volume),
            "relative_volume"
        )
        ...
        results['by_vwap_extension'] = self._analyze_factor(
            completed,
            lambda r: self._bucket_vwap(r.vwap_extension),
            "vwap_extension"
        )
        ...
        results['by_premarket'] = self._analyze_factor(
            completed,
            lambda r: "PREMARKET" if r.is_premarket else "REGULAR",
            "session"
        )
        ...
        results['by_spread'] = self._analyze_factor(
            completed,
            lambda r: self._bucket_spread(r.spread_percent),
            "spread"
        )
        ...
        results['by_split_status'] = self._analyze_factor(
            completed,
            lambda r: r.split_type if r.days_since_split and r.days_since_split < 30 else "NO_RECENT",
            "split_status"
        )
        ...
        results['by_market_trend'] = self._analyze_factor(
            completed,
            lambda r: r.market_trend,
            "market_trend"
        )

        # Overall stats
        cont_rate = sum(1 for r in completed if r.outcome_label in ['STRONG_CONT', 'CONT']) / len(completed)

        results['summary'] = {
            "total_records": len(completed),
            "continuation_rate": round(cont_rate * 100, 1),
            "avg_outcome_30min": round(statistics.mean([r.outcome_30min for r in completed if r.outcome_30min]), 2),
    ...
    def _analyze_factor(self, records: List[MoverRecord],
                        bucket_fn, factor_name: str) -> List[Dict]:
        ...
        for bucket, recs in buckets.items():
            ...
            max_losses = [r.max_loss_after for r in recs if r.max_loss_after is not None]

            cont_count = sum(1 for r in recs if r.outcome_label in ['STRONG_CONT', 'CONT'])

            results.append({
                "value": bucket,
                "sample_size": len(recs),
                "avg_outcome_15min": round(statistics.mean(outcomes_15), 2) if outcomes_15 else 0,
                "avg_outcome_30min": round(statistics.mean(outcomes_30), 2) if outcomes_30 else 0,
                "avg_outcome_1hr": round(statistics.mean(outcomes_1h), 2) if outcomes_1h else 0,
                "continuation_rate": round(cont_count / len(recs) * 100, 1),
                "avg_max_gain": round(statistics.mean(max_gains), 2) if max_gains else 0,
                "avg_max_loss": round(statistics.mean(max_losses), 2) if max_losses else 0
            })
    ...
    def _bucket_rvol(self, rvol: float) -> str:
        if rvol >= 5: return "5x+"
        if rvol >= 3: return "3-5x"
        if rvol >= 2: return "2-3x"
        if rvol >= 1: return "1-2x"
        return "<1x"

    def _bucket_float_rotation(self, rot: float) -> str:
        if rot >= 2: return "2x+ float"
        if rot >= 1: return "1-2x float"
        if rot >= 0.5: return "0.5-1x float"
        return "<0.5x float"

    def _bucket_vwap(self, ext: float) -> str:
        if ext >= 15: return "15%+ above"
        if ext >= 5: return "5-15% above"
        if ext >= 0: return "0-5% above"
        if ext >= -5: return "0-5% below"
        return "5%+ below"

    def _bucket_hod(self, pct: float) -> str:
        if pct <= 2: return "Near HOD (0-2%)"
        if pct <= 5: return "Close to HOD (2-5%)"
        if pct <= 10: return "Mid range (5-10%)"
        return "Far from HOD (10%+)"

    def _bucket_spread(self, spread: float) -> str:
        if spread <= 0.5: return "Tight (<0.5%)"
        if spread <= 1: return "Normal (0.5-1%)"
        if spread <= 2: return "Wide (1-2%)"
        return "Very wide (2%+)"
    ...
    def _find_best_factor(self, results: Dict) -> Dict:
        ...
        for factor, data in results.items():
            if factor == 'summary' or not isinstance(data, list):
                continue
            for item in data:
                if item.get('sample_size', 0) >= 5 and item.get('continuation_rate', 0) > best['continuation_rate']:
                    best = {
                        "factor": factor,
                        "value": item['value'],
    ...
    def _find_worst_factor(self, results: Dict) -> Dict:
        ...
        for factor, data in results.items():
            if factor == 'summary' or not isinstance(data, list):
                continue
            for item in data:
                if item.get('sample_size', 0) >= 5 and item.get('continuation_rate', 0) < worst['continuation_rate']:
                    worst = {
                        "factor": factor,
                        "value": item['value'],
                        "continuation_rate": item['continuation_rate'],
    ...
    def get_prediction_score(self, symbol: str, quote: Dict, context: Dict = None) -> Dict:
        """
        ...
        for item in correlations.get('by_change_percent', []):
            if item['value'] == change_bucket and item['sample_size'] >= 5:
                ...
                score += adj
                factors.append(f"Change {change_bucket}: {item['continuation_rate']:.0f}% cont rate")
        ...
        for item in correlations.get('by_rel_volume', []):
            if item['value'] == rvol_bucket and item['sample_size'] >= 5:
                ...
                score += adj
                factors.append(f"RVOL {rvol_bucket}: {item['continuation_rate']:.0f}% cont rate")
        ...
        for item in correlations.get('by_vwap_extension', []):
            if item['value'] == vwap_bucket and item['sample_size'] >= 5:
                ...
                score += adj
                factors.append(f"VWAP {vwap_bucket}: {item['continuation_rate']:.0f}% cont rate")
        ...
        for item in correlations.get('by_news_heat', []):
            if item['value'] == news_heat and item['sample_size'] >= 5:
                ...
                score += adj
                factors.append(f"News {news_heat}: {item['continuation_rate']:.0f}% cont rate")
        ...
        for item in correlations.get('by_macd', []):
            if item['value'] == macd and item['sample_size'] >= 5:
                ...
                score += adj
                factors.append(f"MACD {macd}: {item['continuation_rate']:.0f}% cont rate")
