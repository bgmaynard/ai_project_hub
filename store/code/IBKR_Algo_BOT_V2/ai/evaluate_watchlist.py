...
def main():
    ...
    candidates = [
        ...
        'YCBD', 'EVTV', 'LHAI', 'ATHA', 'ADTX',
        # Popular penny/momentum stocks
        'FFIE', 'MULN', 'SOUN', 'PLTR', 'SOFI',
        # Quantum/Tech movers
        'IONQ', 'RGTI', 'QBTS', 'RKLB',
        # Crypto miners
        'MARA', 'RIOT', 'CLSK', 'BITF',
    ...
    for symbol in candidates:
        try:
            ...
            status = 'TRADE' if signal in ['BULLISH', 'STRONG_BULLISH'] else 'SKIP' if signal in ['BEARISH', 'STRONG_BEARISH'] else 'WATCH'
            print(f"{symbol:<6} ${price:>8.2f} | {signal:<16} | prob={prob_up:>5.0%} | exp_ret={exp_ret:>+5.1f}% | {status}")
    ...
    print(f"{'Symbol':<8} {'Price':>8} {'Signal':<16} {'Prob Up':>8} {'Exp Ret':>8} {'Float':>8}")
    print("-" * 65)
    ...
    if bullish:
        ...
        for r in bullish[:10]:
            ...
            print(f"{r['symbol']:<8} ${r['price']:>7.2f} {r['signal']:<16} {r['prob_up']:>7.0%} {r['exp_ret']:>+7.1f}% {float_str:>8}")
    else:
