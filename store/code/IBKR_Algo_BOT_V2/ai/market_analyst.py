...
class MarketAnalyst:
    ...
    async def analyze_single_stock(
        ...
        try:
            ...
            analysis = self._parse_analysis_response(symbol, timeframe, claude_response)
...
Keep it practical and actionable for day traders. Be direct and concise."""
    ...
    def _parse_analysis_response(
        self, symbol: str, timeframe: str, claude_response: str
    ) -> Dict:
    ...
    async def analyze_market(
        self, symbols: List[str], market_data: Dict, news_context: Optional[str] = None
    ) -> Dict:
        """
...
async def simple_market_check(symbols: List[str]) -> str:
    ...
    if not analyst.client:
        return (
            "⚠️ Claude API not configured. Set ANTHROPIC_API_KEY environment variable."
        )

    # Mock data - would come from IBKR in production
    market_data = {
        symbol: {"price": 0.0, "change": 0.0, "volume": 0} for symbol in symbols
    }
