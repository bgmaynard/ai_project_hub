...
def analyze_categorical(trades: List[Dict], field: str) -> Dict:
    ...
    results = defaultdict(lambda: {'wins': 0, 'losses': 0, 'total_pnl': 0.0, 'trades': 0})
...
def analyze_numeric(trades: List[Dict], field: str, buckets: int = 5) -> Dict:
    ...
    bucket_results = defaultdict(lambda: {'wins': 0, 'losses': 0, 'total_pnl': 0.0, 'trades': 0, 'values': []})
...
def analyze_boolean(trades: List[Dict], field: str) -> Dict:
    ...
    results = {True: {'wins': 0, 'losses': 0, 'total_pnl': 0.0, 'trades': 0},
...
def generate_correlation_report(trades: List[Dict] = None) -> Dict:
    ...
    if trades_with_triggers:
        ...
        report['technicals'] = {
            ...
            'momentum_strength': analyze_numeric(trades_with_triggers, 'momentum_strength'),
            'volume_spike_percent': analyze_numeric(trades_with_triggers, 'volume_spike_percent'),
            'distance_from_hod': analyze_numeric(trades_with_triggers, 'distance_from_hod'),
            'distance_from_lod': analyze_numeric(trades_with_triggers, 'distance_from_lod'),
        ...
        report['technicals']['vwap_position'] = analyze_categorical(trades_with_triggers, 'vwap_position')
        report['technicals']['macd_signal'] = analyze_categorical(trades_with_triggers, 'macd_signal')

        # Catalyst analysis
        report['catalysts'] = {
            'has_news_catalyst': analyze_boolean(trades_with_triggers, 'has_news_catalyst'),
        ...
        # Correlations
        numeric_fields = [
            'spread_at_entry', 'momentum_strength', 'volume_spike_percent',
            'distance_from_hod', 'distance_from_lod', 'rsi_at_entry',
            'float_rotation', 'relative_volume', 'day_change_at_entry'
        ]
...
def generate_recommendations(report: Dict) -> List[str]:
    ...
    if isinstance(timing.get('time_of_day'), dict):
        ...
        best_times = [k for k, v in tod.items() if v.get('win_rate', 0) > 50 and v.get('trades', 0) >= 3]
        worst_times = [k for k, v in tod.items() if v.get('win_rate', 0) < 30 and v.get('trades', 0) >= 3]

        if best_times:
            recommendations.append(f"TIMING: Best time periods: {', '.join(best_times)}")
        if worst_times:
            recommendations.append(f"TIMING: Avoid trading during: {', '.join(worst_times)}")
    ...
    if stop_loss_stats.get('trades', 0) > 5:
        ...
        recommendations.append(f"STOP LOSS: {stop_loss_stats['trades']} trades hit stop loss (${stop_pnl:.2f})")

    trailing_stats = exit_reasons.get('TRAILING_STOP', {})
    if trailing_stats.get('trades', 0) > 0:
        trail_pnl = trailing_stats.get('total_pnl', 0)
        recommendations.append(f"TRAILING: {trailing_stats['trades']} trailing stops (${trail_pnl:.2f})")
    ...
    if isinstance(correlations, dict) and correlations:
        for field, corr in correlations.items():
            if isinstance(corr, (int, float)) and abs(corr) > 0.3:
                ...
                recommendations.append(f"CORRELATION: {field} has {direction} correlation ({corr:.2f}) with P&L")

    # Check symbols
    symbols = report.get('symbols', {})
    if symbols:
        profitable_symbols = [s for s, stats in symbols.items() if stats.get('total_pnl', 0) > 5]
        losing_symbols = [s for s, stats in symbols.items() if stats.get('total_pnl', 0) < -10]

        if profitable_symbols:
            recommendations.append(f"SYMBOLS: Profitable: {', '.join(profitable_symbols[:5])}")
        if losing_symbols:
            recommendations.append(f"SYMBOLS: Avoid/review: {', '.join(losing_symbols[:5])}")
    ...
    if win_rate < 35:
        recommendations.append("STRATEGY: Win rate below 35% - consider tightening entry criteria")
    elif win_rate > 50:
        recommendations.append("STRATEGY: Win rate above 50% - strategy is working well")

    if not recommendations:
        recommendations.append("Need more trades with secondary triggers for detailed analysis")
...
def print_report(report: Dict):
    ...
    if isinstance(timing.get('time_of_day'), dict) and timing['time_of_day']:
        ...
        for period, stats in timing['time_of_day'].items():
            print(f"  {period:20} | {stats['trades']:3} trades | {stats['win_rate']:5.1f}% win | ${stats['total_pnl']:7.2f}")

    if isinstance(timing.get('day_of_week'), dict) and timing['day_of_week']:
        print(f"\n--- DAY OF WEEK ---")
        for day, stats in timing['day_of_week'].items():
            print(f"  {day:20} | {stats['trades']:3} trades | {stats['win_rate']:5.1f}% win | ${stats['total_pnl']:7.2f}")
    ...
    if exit_reasons:
        ...
        for reason, stats in exit_reasons.items():
            print(f"  {reason:20} | {stats['trades']:3} trades | {stats['win_rate']:5.1f}% win | ${stats['total_pnl']:7.2f}")
    ...
    if entry_signals:
        ...
        for signal, stats in entry_signals.items():
            print(f"  {signal:20} | {stats['trades']:3} trades | {stats['win_rate']:5.1f}% win | ${stats['total_pnl']:7.2f}")
    ...
    if isinstance(correlations, dict) and correlations:
        ...
        for field, corr in correlations.items():
            if isinstance(corr, (int, float)):
                bar = "+" * int(abs(corr) * 10) if corr > 0 else "-" * int(abs(corr) * 10)
    ...
    if symbols:
        ...
        for i, (symbol, stats) in enumerate(list(symbols.items())[:5]):
            print(f"  {symbol:6} | {stats['trades']:3} trades | {stats['win_rate']:5.1f}% win | ${stats['total_pnl']:7.2f}")
