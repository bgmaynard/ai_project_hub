...
MOMENTUM_SYMBOLS = {
    ...
    'recent_movers': [
        'LHAI', 'NRXS', 'JLHL', 'WULF',  # From watchlist
        'ATHA', 'EVTV',  # Recent trades
        'SOUN', 'IONQ', 'RGTI', 'QUBT', 'RKLB',  # Quantum/AI hype
    ],

    # Penny stocks / Low priced momentum
    'penny_momentum': [
        'MULN', 'FFIE', 'GOEV', 'NKLA', 'LCID',  # EV pennies
        'BBIG', 'ATER', 'PROG', 'SDC', 'WISH',   # Meme/squeeze plays
        'CLOV', 'CLVS', 'OCGN', 'SRNE', 'VXRT',  # Biotech pennies
        'GNUS', 'CTRM', 'NAKD', 'SNDL', 'TLRY',  # Other small caps
    ],

    # SPACs and recent de-SPACs (volatile)
    'spacs': [
        'DWAC', 'BKKT', 'DNA', 'GRAB', 'SOFI',
        'OPEN', 'PLTR', 'HOOD', 'AFRM', 'UPST',
        'RIVN', 'LCID', 'PTRA', 'FSR', 'RIDE',
    ],

    # Small-cap biotech (catalyst driven)
    'biotech_small': [
        'NVAX', 'MRNA', 'BNTX', 'INO', 'VXRT',
        'SAVA', 'AGEN', 'MRIN', 'IMVT', 'CTXR',
        'CRBP', 'FBIO', 'VERU', 'ATHX', 'APLS',
    ],

    # Small-cap tech/growth
    'tech_small': [
        'BIGC', 'FVRR', 'FSLY', 'NET', 'DDOG',
        'ZM', 'DOCU', 'PTON', 'CHWY', 'ETSY',
        'FUBO', 'SKLZ', 'PUBM', 'MGNI', 'TTD',
    ],

    # Cannabis (volatile sector)
    'cannabis': [
        'SNDL', 'TLRY', 'CGC', 'ACB', 'CRON',
        'HEXO', 'OGI', 'VFF', 'GRWG', 'CURLF',
    ],

    # Chinese ADRs (volatile)
    'china_adr': [
        'NIO', 'XPEV', 'LI', 'BABA', 'JD',
        'PDD', 'BIDU', 'BILI', 'IQ', 'TME',
    ],

    # Energy/Mining small caps
    'energy_small': [
        'INDO', 'IMPP', 'PTRA', 'EVGO', 'CHPT',
        'PLUG', 'FCEL', 'BE', 'BLDP', 'HYLN',
    ],

    # High beta / Leveraged exposure
    'high_beta': [
        'TQQQ', 'SQQQ', 'SPXL', 'SPXU', 'UVXY',
        'VXX', 'LABU', 'LABD', 'SOXL', 'SOXS',
    ],
}
...
class MomentumDataCollector:
    ...
    def fetch_and_filter(
        ...
        logger.info(f"Filters: price ${min_price}-${max_price}, min vol {min_avg_volume:,}")
...
class CorrelationAnalyzer:
    ...
    def analyze(
        ...
        for r in self.correlations['top_positive'][:10]:
            logger.info(f"  {r['feature']}: r={r['spearman_r']:.3f} (p={r['spearman_p']:.4f})")
...
class MomentumTrainer:
    ...
    def prepare_data(
        ...
        for symbol, df in data.items():
            try:
                ...
                for col in ['momentum_5', 'momentum_10', 'vol_spike', 'price_accel', 'gap_pct']:
                    if col in df_prep.columns:
                        # Align index
                        features[col] = df_prep.loc[features.index, col] if col in df_prep else 0
    ...
    def _add_momentum_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        data['close_position'] = (data['Close'] - data['Low']) / (data['High'] - data['Low'] + 0.0001)
    ...
    def train(
        ...
        for fold, (train_idx, val_idx) in enumerate(tscv.split(X)):
            ...
            logger.info(f"  Fold {fold + 1}: Acc={val_acc:.3f}, AUC={val_auc:.3f}, Prec={val_precision:.3f}, Rec={val_recall:.3f}")
    ...
    def save(self, model_path: str = "ai/momentum_model.pkl", meta_path: str = "ai/momentum_model_meta.json"):
        """Save momentum model"""
        import pickle
...
class MomentumTrainingPipeline:
    ...
    def _print_summary(self):
        ...
        print(f"\nCorrelation Analysis:")
        print(f"  Significant features: {corr.get('significant_count', 0)}/{corr.get('total_features', 0)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--horizon", type=int, default=3, help="Prediction horizon days")
