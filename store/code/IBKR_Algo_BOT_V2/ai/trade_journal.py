...
class TradeJournal:
    ...
    def log_trade(self, trade: TradeEntry) -> bool:
        ...
        try:
            ...
            """, (
                trade.id, trade.trade_date, trade.symbol, trade.entry_time,
                trade.entry_price, trade.entry_reason, trade.exit_time, trade.exit_price,
                trade.exit_reason, trade.quantity, trade.position_size_usd, trade.direction,
                trade.strategy, trade.pattern, trade.setup_quality, trade.ai_signal,
                trade.ai_confidence, trade.ai_prediction, trade.gap_percent, trade.relative_volume,
                trade.float_shares, trade.market_cap, trade.sector, trade.stop_loss,
                trade.take_profit, trade.risk_reward_ratio, trade.pnl, trade.pnl_percent,
                1 if trade.is_winner else 0, trade.hold_time_minutes, trade.max_drawdown,
                trade.max_profit, trade.slippage, trade.market_trend, trade.spy_change,
                trade.vix_level, trade.notes, trade.tags, trade.mistakes, trade.lessons,
                1 if trade.paper_trade else 0, trade.created_at
            ))

            conn.commit()
            conn.close()

            logger.info(f"[JOURNAL] Logged trade: {trade.id} - {trade.symbol} - P&L: ${trade.pnl:.2f}")
            return True
    ...
    def get_performance_metrics(self,
        ...
        profit_factor = gross_profit / gross_loss if gross_loss > 0 else 999.99  # Use finite value for JSON
        ...
        for s in strategy_stats:
            strategy_stats[s]['win_rate'] = (
                ...
                if strategy_stats[s]['trades'] > 0 else 0
            )
        ...
        return {
            ...
            "risk_metrics": {
                ...
                "avg_risk_reward": round(
                    statistics.mean([t['risk_reward_ratio'] for t in trades if t['risk_reward_ratio']])
                    if any(t['risk_reward_ratio'] for t in trades) else 0, 2
                )
            },
            "strategy_performance": strategy_stats,
            "pattern_performance": pattern_stats,
            "ai_accuracy": {
                "bullish_signals": len([t for t in trades if t['ai_signal'] == 'BULLISH']),
                "bullish_wins": len([t for t in trades if t['ai_signal'] == 'BULLISH' and t['is_winner']]),
                "avg_ai_confidence_winners": round(
                    statistics.mean([t['ai_confidence'] for t in winners if t['ai_confidence']])
                    if any(t['ai_confidence'] for t in winners) else 0, 2
                ),
                "avg_ai_confidence_losers": round(
                    statistics.mean([t['ai_confidence'] for t in losers if t['ai_confidence']])
                    if any(t['ai_confidence'] for t in losers) else 0, 2
                )
            }
        }
    ...
    def run_daily_analysis(self, date: str = None) -> Dict:
        ...
        for strat, data in analysis['strategies_used'].items():
            if data['count'] >= 2:  # At least 2 trades
                ...
                if win_rate >= 60:
                    analysis['what_worked'].append(f"{strat}: {win_rate:.0f}% win rate ({data['count']} trades)")
                elif win_rate < 40:
                    analysis['what_didnt_work'].append(f"{strat}: {win_rate:.0f}% win rate ({data['count']} trades)")
        ...
        analysis['common_mistakes'] = [
            f"{m} ({c} times)" for m, c in sorted(mistakes_count.items(), key=lambda x: -x[1])[:5]
        ]
    ...
    def _generate_recommendations(self, trades: List[Dict], analysis: Dict) -> List[str]:
        ...
        # Average win vs loss
        avg_win = abs(analysis.get('metrics', {}).get('pnl_metrics', {}).get('avg_win', 0))
        avg_loss = abs(analysis.get('metrics', {}).get('pnl_metrics', {}).get('avg_loss', 0))
        if avg_loss > avg_win:
            recommendations.append({
        ...
        if ai_data.get('bullish_signals', 0) > 0:
            ai_win_rate = (ai_data.get('bullish_wins', 0) / ai_data.get('bullish_signals', 1)) * 100
            if ai_win_rate < 50:
                recommendations.append({
                    "category": "ai_signals",
                    "priority": "MEDIUM",
    ...
    def analyze_trends(self, days: int = 30) -> Dict:
        ...
        for t in trades:
            ...
            week_start = (trade_date - timedelta(days=trade_date.weekday())).strftime('%Y-%m-%d')

            if week_start not in weekly_stats:
                weekly_stats[week_start] = {
                    'trades': 0, 'wins': 0, 'pnl': 0, 'strategies': {}
                }
        ...
        wr_trend = [
            ...
            if weekly_stats[w]['trades'] > 0 else 0
            for w in weeks
        ]
        ...
        return {
            ...
            "weekly_breakdown": {
                w: {
                    ...
                    "win_rate": round(weekly_stats[w]['wins'] / weekly_stats[w]['trades'] * 100, 1)
                              if weekly_stats[w]['trades'] > 0 else 0,
            ...
            "trends": {
                ...
                "consistency": "HIGH" if statistics.stdev(pnl_trend) < 100 else "LOW" if len(pnl_trend) > 1 else "N/A"
            },
            "insights": self._generate_trend_insights(trades, weekly_stats)
        }

    def _generate_trend_insights(self, trades: List[Dict], weekly_stats: Dict) -> List[str]:
        ...
        if weekly_stats:
            ...
            insights.append(f"Best week: {best_week[0]} with ${best_week[1]['pnl']:.2f} P&L")

            worst_week = min(weekly_stats.items(), key=lambda x: x[1]['pnl'])
            if worst_week[1]['pnl'] < 0:
                insights.append(f"Worst week: {worst_week[0]} with ${worst_week[1]['pnl']:.2f} loss")
        ...
        if strategy_pnl:
            ...
            insights.append(f"Most profitable strategy: {best_strat[0]} (${best_strat[1]:.2f})")
