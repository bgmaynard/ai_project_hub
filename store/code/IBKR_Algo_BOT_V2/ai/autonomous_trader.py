...
from .alpha_fusion_v2 import (AlphaFusionEngine, MarketData, TradeLabel,
                              market_data_from_ibkr)
from .trading_engine import (OrderType, RiskLimits, Side, TradingEngine,
                             TradingSignal)

# Optional sentiment analyzer
try:
    from .warrior_sentiment_analyzer import (WarriorSentimentAnalyzer,
...
try:
    from .momentum_scanner import CRITERIA as SCANNER_CRITERIA
    from .momentum_scanner import get_momentum_scanner
...
class AutonomousTrader:
    ...
    def __init__(self, config: BotConfig, ib_connection=None):
        ...
        self.trading_engine = TradingEngine(
            account_size=config.account_size, risk_limits=risk_limits
        )
        ...
        if HAS_MOMENTUM_SCANNER and config.scanner_enabled:
            try:
                ...
                logger.info(
                    "✓ Momentum scanner integrated for autonomous stock discovery"
                )
            except Exception as e:
        ...
        self.scanner_discovered_stocks: Dict[str, Dict] = (
            {}
        )  # symbol -> {score, catalyst, timestamp}
        ...
        self.position_entry_times: Dict[str, float] = (
            {}
        )  # symbol -> timestamp for time-based exits
    ...
    async def _trading_loop(self):
        ...
        while self.running:
            try:
                ...
                all_symbols = set(self.config.watchlist) | set(
                    self.scanner_discovered_stocks.keys()
                )
    ...
    async def _refresh_watchlist_from_scanner(self):
        ...
        try:
            ...
            for sym, data in self.scanner_discovered_stocks.items():
                ...
                if (
                    data.get("score", 0) < self.config.scanner_min_score
                    or age_hours > 1
                ):
            ...
            for stock in top_stocks:
                ...
                if (
                    len(self.scanner_discovered_stocks)
                    >= self.config.scanner_max_watchlist
                ):
                    # Only replace if new stock has higher score than lowest
                    if self.scanner_discovered_stocks:
                        min_sym = min(
                            self.scanner_discovered_stocks.keys(),
                            key=lambda s: self.scanner_discovered_stocks[s].get(
                                "score", 0
                            ),
                        )
                        min_score = self.scanner_discovered_stocks[min_sym].get(
                            "score", 0
                        )
                        if score > min_score:
                ...
                if symbol not in self.scanner_discovered_stocks:
                    ...
                    added_count += 1
                    logger.info(
                        f"SCANNER DISCOVERY: {symbol} (Score: {score:.0f}, "
                        f"Change: {stock.get('change_pct', 0):+.1f}%, Catalyst: {catalyst})"
                    )
                else:
                    # Update existing
                    self.scanner_discovered_stocks[symbol]["score"] = score
                    self.scanner_discovered_stocks[symbol]["price"] = stock.get(
                        "price", 0
                    )

            if added_count > 0 or removed_count > 0:
                logger.info(
                    f"Scanner refresh: +{added_count} added, -{removed_count} removed, "
    ...
    async def _get_sentiment_score(self, symbol: str) -> float:
        ...
        try:
            ...
            sentiment_result = await self.sentiment_analyzer.analyze_symbol(
                symbol=symbol, hours=6, sources=["news", "twitter", "reddit"]
            )
            ...
            if sentiment_result.signals_count > 0:
                score = (
                    sentiment_result.overall_score * sentiment_result.overall_confidence
                )
                ...
                if sentiment_result.breaking_news:
                    ...
                    logger.info(
                        f"Breaking news for {symbol}: {sentiment_result.breaking_news.headline[:80]}"
                    )
                ...
                logger.debug(
                    f"Sentiment for {symbol}: {score:+.3f} ({sentiment_result.signals_count} signals)"
                )
                return score
            else:
    ...
    def _get_weighted_indicator_signal(
        ...
        try:
            ...
            if (
                hasattr(market_data, "indicators")
                and "macd_histogram" in market_data.indicators
            ):
                    ...
                    signals.append(
                        {
                            ...
                            "direction": "BUY",
                            "confidence": min(0.7, abs(macd_hist) / 2),
                        }
                    )
                elif macd_hist < 0:
                    signals.append(
                ...
                signals.append(
                    {
                        ...
                        "direction": "BUY",
                        "confidence": min(0.6, (volume_ratio - 2) / 5),
                    }
                )
            ...
            if signals:
                ...
                logger.debug(
                    f"Weighted signal for {symbol}: {result['direction']} (score={result['composite_score']:.3f})"
                )
                return result
            else:
                return {
    ...
    def _record_prediction(
        ...
        try:
            ...
            logger.debug(
                f"Recorded prediction {prediction_id} for {symbol}: {direction} @ ${price:.2f}"
            )
    ...
    async def _process_symbol(self, symbol: str):
        ...
        try:
            ...
            combined_sentiment = (
                sentiment * 0.4 + weighted_signal["composite_score"] * 0.6
            )
            ...
            if signal:
                ...
                logger.info(
                    f"Signal generated for {symbol}: {signal.side.value} {signal.quantity} @ {signal.confidence:.3f}"
                )
                logger.info(
                    f"  Weighted indicator: {weighted_signal['direction']} ({weighted_signal['confidence']:.2f})"
                )
                ...
                price = (
                    market_data.last
                    if market_data.last
                    else (market_data.bid + market_data.ask) / 2
                )
    ...
    async def _execute_signal(
        ...
        try:
            ...
            if signal.order_type == OrderType.MARKET:
                order = MarketOrder(signal.side.value, signal.quantity)
            elif signal.order_type == OrderType.LIMIT:
                order = LimitOrder(
                    signal.side.value, signal.quantity, signal.limit_price
                )
            else:
            ...
            if trade.orderStatus.status == "Filled":
                ...
                intended_price = (
                    ...
                    else market_data.ask if signal.side == Side.BUY else market_data.bid
                )
                ...
                quantity_signed = (
                    signal.quantity if signal.side == Side.BUY else -signal.quantity
                )
                ...
                logger.info(
                    f"✓ Order filled: {signal.symbol} {signal.side.value} {signal.quantity} @ ${filled_price:.2f}"
                )
    ...
    async def _check_exits(self):
        ...
        for symbol in list(self.trading_engine.positions.keys()):
            try:
                ...
                current_price = (
                    float(ticker.last) if ticker.last == ticker.last else None
                )
                ...
                if not exit_signal:
                    ...
                    if position:
                        ...
                        position_age = time.time() - self.position_entry_times.get(
                            symbol, time.time()
                        )

                        # Rule 1: STALE POSITION - not profitable after 5 min = GET OUT
                        if (
                            position_age > self.config.stale_position_seconds
                            and pnl_pct <= 0
                        ):
                            logger.warning(
                                f"STALE POSITION: {symbol} {position_age/60:.1f}min old, {pnl_pct:.1%} - MOMENTUM FAILED"
                            )
                            from .trading_engine import (OrderType, Side,
                                                         TradingSignal)
                        ...
                        # Rule 2: MAX HOLD TIME - 15 min max (unless big winner >3%)
                            ...
                            position_age > self.config.max_hold_seconds
                            and pnl_pct < 0.03
                        ):
                            logger.warning(
                                f"MAX HOLD: {symbol} {position_age/60:.1f}min old - TIME TO EXIT"
                            )
                            from .trading_engine import (OrderType, Side,
                                                         TradingSignal)
    ...
    async def _close_all_positions(self):
        ...
        for symbol in list(self.trading_engine.positions.keys()):
            try:
                ...
                # Create exit signal
                from .trading_engine import OrderType, Side, TradingSignal
