...
# Import AI modules
try:
    from ai.claude_api import AI_AVAILABLE as CLAUDE_AVAILABLE
    from ai.claude_api import get_claude_response
    from ai.claude_api import router as claude_router
except ImportError:
    claude_router = None
...
try:
    from ai.alpha_fusion import L1, predict_one
except ImportError:
    predict_one = None
    L1 = None
...
async def startup_event():
    ...
    if not auto_connect:
        logger.info(
            "IBKR auto-connect disabled (set IBKR_AUTO_CONNECT=true in .env to enable)"
        )
        return
    ...
    def _on_ticker_update(self, tickers):
        ...
        for ticker in tickers:
            ...
            data = {
                ...
                "bid_size": (
                    int(ticker.bidSize) if ticker.bidSize == ticker.bidSize else None
                ),
                "ask_size": (
                    int(ticker.askSize) if ticker.askSize == ticker.askSize else None
                ),
                "volume": (
                    int(ticker.volume) if ticker.volume == ticker.volume else None
                ),
            ...
            if hasattr(ticker, "domBids") and ticker.domBids:
                ...
                if bids or asks:
                    self.l2_data[symbol] = {"bids": bids, "asks": asks}
                    asyncio.create_task(
                        self.broadcast(symbol, "l2", self.l2_data[symbol])
                    )
                    depth_updated = True
                    logger.debug(
                        f"L2 depth for {symbol}: {len(bids)} bids, {len(asks)} asks"
                    )
                    ...
                    bids.append(
                        {
                            ...
                            "size": (
                                data["bid_size"]
                                if i == 0
                                else int(100 + (hash(f"{symbol}{i}") % 400))
                            ),
                            "market_maker": ["NSDQ", "ARCA", "BATS", "EDGX", "IEX"][
                                i % 5
                            ],
                        }
                    )
                    ...
                    asks.append(
                        {
                            ...
                            "size": (
                                data["ask_size"]
                                if i == 0
                                else int(100 + (hash(f"{symbol}{i+10}") % 400))
                            ),
                            "market_maker": ["NYSE", "CBOE", "NSDQ", "ARCA", "BATS"][
                                i % 5
                            ],
                        }
                    )
...
async def connect_ibkr(request: IBKRConnectionRequest):
    """Connect to IBKR TWS/Gateway"""
    try:
        result = await data_bus.connect_ibkr(
            request.host, request.port, request.client_id
        )
        return result
    except Exception as e:
...
async def get_ibkr_status():
    ...
    return {"connected": data_bus.connected, "available": IBKR_AVAILABLE}
    ...
    try:
        ...
        for item in account_values:
            if item.tag == "AccountType":
                ...
                account_info["day_trades_remaining"] = (
        ...
        if "MARGIN" in acct_type or "REG T" in acct_type:
            if net_liq < 25000:
                ...
                if day_trades_left is not None and day_trades_left <= 0:
                    ...
                    logger.warning(
                        f"PDT WARNING: Only {day_trades_left} day trade(s) remaining!"
                    )
        ...
        if "CASH" in acct_type:
            ...
            logger.warning(
                "CASH ACCOUNT: Be aware of T+2 settlement rules. Avoid good faith violations."
            )
        ...
        if order.order_type == "MARKET":
            ...
            if order.limit_price is None or (
                isinstance(order.limit_price, float)
                and (order.limit_price != order.limit_price)
            ):  # Check for None or NaN
                raise HTTPException(
                    status_code=400,
                    detail="Valid limit price required for LIMIT orders",
                )
            ...
            if order.limit_price is None or order.stop_price is None:
                raise HTTPException(
                    status_code=400,
                    detail="Limit and stop price required for STOP_LIMIT orders",
                )
            ib_order = StopLimitOrder(
                order.action, order.quantity, order.limit_price, order.stop_price
            )
        else:
            raise HTTPException(
                status_code=400, detail=f"Invalid order type: {order.order_type}"
            )
        ...
        logger.info(
            f"Order placed: {order.action} {order.quantity} {order.symbol} @ {order.order_type}"
        )
...
async def subscribe_symbol(request: SymbolSubscription):
    ...
    try:
        ...
        if "l2" in request.data_types:
            data_bus.ibkr_connector.request_market_depth(
                request.symbol, request.exchange, depth=10
            )
...
async def get_price_old(symbol: str):
    ...
    if symbol in data_bus.market_data:
        ...
        raise HTTPException(
            status_code=404,
            detail=f"No data for {symbol}. Subscribe first via /api/subscribe",
        )
...
async def init_bot(config: Optional[BotConfigUpdate] = None):
    ...
    if not data_bus.connected or not data_bus.ib:
        raise HTTPException(
            status_code=503, detail="IBKR not connected. Please connect first."
        )
    ...
    try:
        ...
        # Create bot instance
        autonomous_bot = AutonomousTrader(config=bot_config, ib_connection=data_bus.ib)
...
async def start_bot():
    ...
    if autonomous_bot is None:
        raise HTTPException(
            status_code=400, detail="Bot not initialized. Use /api/bot/init first."
        )
...
async def get_account():
    ...
    try:
        ...
        # Get account number
        account_id = (
            data_bus.ib.managedAccounts()[0]
            if data_bus.ib.managedAccounts()
            else "Unknown"
        )
        ...
        for item in account_values:
            if item.tag == "NetLiquidation":
                ...
                summary["day_trades_remaining"] = (
                    int(item.value) if item.value else None
                )
            elif item.tag == "DayTradesRemainingT+1":
                summary["day_trades_remaining_t1"] = (
                    int(item.value) if item.value else None
                )
            elif item.tag == "DayTradesRemainingT+2":
                summary["day_trades_remaining_t2"] = (
                    int(item.value) if item.value else None
                )
            elif item.tag == "DayTradesRemainingT+3":
                summary["day_trades_remaining_t3"] = (
                    int(item.value) if item.value else None
                )
            elif item.tag == "DayTradesRemainingT+4":
                summary["day_trades_remaining_t4"] = (
                    int(item.value) if item.value else None
                )
        ...
        if "MARGIN" in acct_type or "REG T" in acct_type:
            if net_liq >= 25000:
                summary["account_capabilities"] = [
                    "MARGIN",
                    "SHORT_SELLING",
                    "UNLIMITED_DAY_TRADING",
                ]
            ...
            positions.append(
                {
                    ...
                    "market_value": (
                        float(pos.position * pos.marketValue)
                        if hasattr(pos, "marketValue")
                        else 0
                    ),
                    "unrealized_pnl": (
                        float(pos.unrealizedPNL) if hasattr(pos, "unrealizedPNL") else 0
                    ),
                }
            )
    ...
    try:
        ...
        if ai_predictor:
            ...
            result = ai_predictor.train(
                symbol=config.symbol, period=config.period, test_size=0.2
            )
            ...
            logger.info(
                f"Training completed for {training_id}: {result['metrics']['accuracy']:.2%} accuracy"
            )
        else:
...
async def get_training_status(training_id: str):
    ...
    return {"success": True, "session": training_sessions[training_id]}
...
async def configure_alert(config: AlertConfig):
    ...
    alert = {"id": alert_id, "created_at": datetime.now().isoformat(), **config.dict()}
...
async def save_indicator(indicator: Dict[str, Any]):
    ...
    saved_indicators.append(
        {"id": indicator_id, "created_at": datetime.now().isoformat(), **indicator}
    )
...
async def add_bulk_to_worklist(items: List[WorklistItem]):
    ...
    return {
        "success": True,
        "data": {"added": added, "errors": errors, "count": len(added)},
    }
...
async def get_scanner_presets():
    ...
    presets = [
        ...
        },
        {
            "id": "TOP_PERC_LOSE",
            "name": "Top % Losers",
            "description": "Stocks with highest % losses",
        },
        {
            "id": "MOST_ACTIVE",
            "name": "Most Active",
            "description": "Stocks with highest volume",
        },
        {
            "id": "HOT_BY_VOLUME",
            "name": "Hot by Volume",
        ...
        {
            "id": "TOP_TRADE_COUNT",
            "name": "Top Trade Count",
            "description": "Most traded stocks",
        },
        {
            "id": "HOT_BY_PRICE",
            "name": "Hot by Price",
            "description": "Stocks with largest price moves",
        },
        {
            "id": "TOP_PRICE_RANGE",
            "name": "Top Price Range",
            "description": "Largest intraday range",
        },
        {
            "id": "HOT_BY_OPT_VOLUME",
            "name": "Hot by Option Volume",
            "description": "Unusual option volume",
        },
    ]
...
async def run_ibkr_scanner(request: Dict[str, Any]):
    ...
    if data_bus.ib:
        try:
            ...
            logger.info(
                f"Scanner: IBKR connection check - data_bus.connected={data_bus.connected}, ib.isConnected()={is_connected}"
            )
        except:
            is_connected = False
    ...
    try:
        ...
        if filter_options:
            scanner_data = await data_bus.ib.reqScannerDataAsync(
                sub, [], filter_options
            )
        else:
                    ...
                    results.append(
                        {
                            ...
                            "distance": (
                                data.distance if hasattr(data, "distance") else 0
                            ),
                            "benchmark": (
                                data.benchmark if hasattr(data, "benchmark") else 0
                            ),
                            "projection": (
                                data.projection if hasattr(data, "projection") else 0
                            ),
...
async def export_daily_review(request: Dict[str, Any]):
    ...
    if export_format == "json":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=daily_review_{date}.json"
            },
        )
    else:  # PDF
        # Mock PDF generation
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=daily_review_{date}.pdf"
            },
        )
...
async def health_check():
    ...
    return {
        ...
        "ai_predictor_loaded": ai_predictor is not None
        and (ai_predictor.model is not None if ai_predictor else False),
        "claude_available": CLAUDE_AVAILABLE
        and market_analyst is not None
        and (market_analyst.client is not None if market_analyst else False),
        ...
        "active_subscriptions": sum(
            len(subs) for subs in data_bus.subscribers.values()
        ),
