...
WATCHLIST = [
    'LHAI', 'YCBD', 'OCGN', 'MIRA', 'WULF', 'RIOT', 'AMST', 'DRUG',
    'GEVO', 'BITF', 'MARA', 'SOUN', 'KULR', 'LCID', 'CLSK', 'BTDR',
    'ATHA', 'NVAX', 'VCNX', 'SLRX', 'NISN', 'CABA', 'BHAT', 'CANG',
    'SPCB', 'COSM', 'FGEN', 'XTIA', 'CNEY', 'GRPN', 'WKEY'
]
...
while True:
    ...
    if alerts:
        ...
        for a in alerts:
            print(f"  >> {a['symbol']} ${a['price']:.2f} +{a['change']:.1f}% RVol:{a['rvol']:.1f}x")
        print(f"{'!'*60}\n")
    else:
        if scan_count % 6 == 0:  # Status every ~3 min
            print(f"[{now.strftime('%H:%M:%S')}] Scan #{scan_count} - No spikes. Market flat.")
