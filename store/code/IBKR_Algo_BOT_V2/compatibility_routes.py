...
_news_cache = (
    {}
)  # symbol -> {"has_news": bool, "news_age_hours": float, "catalyst_type": str, "timestamp": time}
_NEWS_CACHE_TTL = 300  # Cache news for 5 minutes

# FinViz news cache - populated from scanner top plays
_finviz_news_cache = (
    {}
)  # symbol -> {"catalyst_type": str, "change_pct": float, "scan_time": datetime}
_FINVIZ_CACHE_TTL = 600  # Cache FinViz data for 10 minutes

# AI Prediction cache
_prediction_cache = (
    {}
)  # symbol -> {"signal": str, "confidence": float, "prob_up": float, "timestamp": time}
_PREDICTION_CACHE_TTL = 600  # Cache predictions for 10 minutes

# Technical metrics cache (VWAP, MACD, relative volume, etc.)
_technical_cache = (
    {}
)  # symbol -> {"vwap": float, "macd_signal": str, "rel_vol": float, "timestamp": time}
...
async def _update_finviz_news_cache():
    ...
    try:
        ...
        if scanner.candidates:
            ...
            logger.info(
                f"[FINVIZ] Updated news cache with {len(_finviz_news_cache)} movers"
            )
    except Exception as e:
                            ...
                            if created:
                                try:
                                    ...
                                    if isinstance(created, str):
                                        ...
                                        news_age_hours = (
                                            datetime.now(news_time.tzinfo) - news_time
                                        ).total_seconds() / 3600
                                except:
                                    news_age_hours = 2
                            break
...
def _get_prediction_for_symbol(symbol: str) -> dict:
    ...
    result = {"signal": "N/A", "confidence": 0.0, "prob_up": 0.5, "timestamp": now}
    ...
    if predictor and predictor.model is not None:
        try:
            ...
            result = {
                ...
                "confidence": round(
                    prediction.get("confidence", 0) * 100, 1
                ),  # As percentage
                "prob_up": round(
                    prediction.get("prob_up", 0.5) * 100, 1
                ),  # As percentage
                "timestamp": now,
            }
...
def _get_technical_metrics(symbol: str, quote: dict) -> dict:
    ...
    try:
        ...
        if high > 0 and low > 0:
            ...
            vwap_ext = (
                ((price - vwap_estimate) / vwap_estimate * 100)
                if vwap_estimate > 0
                else 0
            )
...
async def get_positions():
    ...
    try:
            ...
            formatted.append(
                {
                    ...
                    "current_price": pos.get(
                        "currentPrice",
                        pos.get("marketValue", 0) / max(pos.get("quantity", 1), 1),
                    ),
        ...
        return {"success": True, "positions": formatted, "count": len(formatted)}
    except Exception as e:
...
async def get_level2_compat(symbol: str):
    ...
    try:
        ...
        return {
            "symbol": symbol,
            "bids": [
                {"price": quote.get("bid", 0), "size": quote.get("bid_size", 100)}
            ],
            "asks": [
                {"price": quote.get("ask", 0), "size": quote.get("ask_size", 100)}
            ],
            "source": "cache",
            "luld": luld,
        }
...
async def get_timesales_compat(symbol: str, limit: int = 50):
    ...
    return {
        ...
        "available_sources": ["Finnhub Premium", "Polygon.io", "Interactive Brokers"],
    }
...
async def get_worklist_compat():
    ...
    now = time.time()
    if (
        _worklist_cache["data"]
        and (now - _worklist_cache["timestamp"]) < _WORKLIST_CACHE_TTL
    ):
        logger.info(
            f"[WORKLIST] Cache HIT - returning cached data (age: {(now - _worklist_cache['timestamp'])*1000:.0f}ms)"
        )
    ...
    async with _worklist_cache["lock"]:
        ...
        try:
            ...
            t1 = time.time()
            logger.info(
                f"[WORKLIST] Got {len(watchlist_symbols)} symbols in {(t1-start_total)*1000:.0f}ms"
            )
            ...
            if HAS_UNIFIED:
                ...
                try:
                    ...
                    t3 = time.time()
                    logger.info(
                        f"[WORKLIST] Batch fetch {len(watchlist_symbols)} quotes in {(t3-t2)*1000:.0f}ms"
                    )
                        ...
                        symbols.append(
                            {
                                ...
                                "float_formatted": float_data.get(
                                    "float_formatted", "N/A"
                                ),
                    ...
                    asyncio.create_task(
                        _update_news_cache_for_symbols(watchlist_symbols)
                    )
                    ...
                    threading.Thread(
                        target=_update_prediction_cache_for_symbols,
                        args=(watchlist_symbols,),
                        daemon=True,
                    ).start()
                except Exception as batch_err:
                    logger.warning(
                        f"[WORKLIST] Batch fetch failed: {batch_err}, falling back to sequential"
                    )
                                ...
                                symbols.append(
                                    {
                                        ...
                                        **{
                                            k: v
                                            for k, v in _get_news_detail(symbol).items()
                                        },
                                        "ai_signal": ai_pred.get("signal", "N/A"),
                                        "ai_confidence": ai_pred.get("confidence", 0),
                                        "ai_prob_up": ai_pred.get("prob_up", 50),
                                        # Technical Entry Metrics
                                        "catalyst_type": tech.get(
                                            "catalyst_type", "NONE"
                                        ),
                                        "catalyst_color": tech.get(
                                            "catalyst_color", "#666"
                                        ),
                                        "macd_signal": tech.get("macd_signal", "N/A"),
                                        "macd_color": tech.get("macd_color", "#666"),
                                        ...
                                        "vwap_extension": tech.get("vwap_extension", 0),
                                        "vwap_color": tech.get("vwap_color", "#666"),
                                        "percent_from_hod": tech.get(
                                            "percent_from_hod", 0
                                        ),
                                        "hod_color": tech.get("hod_color", "#666"),
                                        "entry_score": tech.get("entry_score", 0),
                                        "entry_color": tech.get("entry_color", "#666"),
                                        "entry_status": tech.get(
                                            "entry_status", "WAIT"
                                        ),
                                        "status_color": tech.get(
                                            "status_color", "#ef4444"
                                        ),
                                        # HALT Status
                                        "is_halted": tech.get("is_halted", False),
                    ...
                    logger.warning(
                        f"[WORKLIST] Sequential fetch took {(time.time()-seq_start)*1000:.0f}ms"
                    )
            else:
            ...
            logger.info(
                f"[WORKLIST] TOTAL request time: {(time.time()-start_total)*1000:.0f}ms for {len(symbols)} symbols"
            )
            return result
        except Exception as e:
            ...
            return {
                "success": True,
                "data": [_get_empty_worklist_item(s) for s in ["SPY", "QQQ", "AAPL"]],
                "count": 3,
            }
...
async def check_symbol_news(symbol: str):
    ...
    return {"symbol": symbol, "has_news": has_news}
    ...
    try:
            ...
            try:
                ...
                if price is not None:
                    if price < MIN_PRICE:
                        return {
                            "success": False,
                            "message": f"REJECTED: {symbol} price ${price:.2f} < ${MIN_PRICE} minimum",
                        }
                    if price > MAX_PRICE:
                        return {
                            "success": False,
                            "message": f"REJECTED: {symbol} price ${price:.2f} > ${MAX_PRICE} maximum",
                        }
        ...
        if HAS_WATCHLIST:
            ...
            if watchlist_id:
                ...
                return {
                    "success": True,
                    "message": f"Symbol {symbol} added to worklist",
                }
    ...
    try:
        ...
        if HAS_WATCHLIST:
            ...
            if watchlist_id:
                ...
                return {
                    "success": True,
                    "message": f"Symbol {symbol} removed from worklist",
                }
                        ...
                        enriched_symbols.append(
                            {
                                ...
                                "change_percent": (
                                    quote.get("change_percent", 0) if quote else 0
                                ),
...
async def get_ai_working_list():
    ...
    try:
        ...
        return {"symbols": enriched, "count": len(enriched), "name": "AI_Working_List"}
    except Exception as e:
        return {"symbols": [], "count": 0, "error": str(e)}
...
async def sync_ai_working_list(watchlist_name: str = "AI_Working_List"):
    ...
    try:
        if HAS_WATCHLIST:
            ...
            return {
                "success": True,
                "synced": len(_ai_working_list),
                "symbols": _ai_working_list,
            }
        return {"success": False, "message": "Watchlist manager not available"}
    except Exception as e:
        return {"success": False, "message": str(e)}
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                "http://localhost:9100/api/scanner/warrior/scan", timeout=30
            ) as resp:
                if resp.status == 200:
...
async def claude_status_compat():
    ...
    try:
        ...
        return {
            ...
            "features": (
                len(getattr(predictor, "feature_names", [])) if predictor else 0
            ),
        }
    except:
        return {"available": False, "status": "offline", "model_loaded": False}
...
async def bot_status_compat():
    """Legacy bot status"""
    return {"status": "stopped", "enabled": False, "trades_today": 0, "pnl_today": 0.0}
...
async def schwab_backtest_run(data: dict = None):
    ...
    try:
        ...
        print(
            f"[BACKTEST API] Received {len(symbols)} symbols: {symbols[:5]}{'...' if len(symbols) > 5 else ''}"
        )
        ...
        try:
            ...
            if date_range < 180:
                print(
                    f"[BACKTEST API] Date range {date_range} days too short, extending to 180"
                )
        ...
        print(
            f"[BACKTEST API] Capital: ${initial_capital}, Position: {position_size_pct*100}%, Max Pos: {max_positions}"
        )
...
async def schwab_backtest_quick(data: dict = None):
    ...
    try:
        ...
        print(
            f"[QUICK BACKTEST] {len(symbols)} symbols from {start_date} to {end_date}"
        )
    ...
    try:
        ...
        return {
            ...
            "models": {
                ...
                "momentum": {
                    "signal": result.get("signal", "NEUTRAL"),
                    "confidence": max(0, result.get("confidence", 0) - 0.1),
                    "weight": 0.3,
                },
                "mean_reversion": {
                    "signal": "NEUTRAL",
...
async def ensemble_predict_batch(data: dict):
    ...
    for symbol in symbols[:20]:
        try:
            ...
            results.append(
                {"symbol": symbol.upper(), "signal": "ERROR", "confidence": 0}
            )
...
async def rl_train(data: dict):
    ...
    _rl_state["best_reward"] = max(
        _rl_state["best_reward"], _rl_state["avg_reward"] * 1.5
    )
    _rl_state["epsilon"] = max(0.01, _rl_state["epsilon"] * 0.995)
    _rl_state["status"] = "trained"
    return {
        "success": True,
        "message": f"Trained {episodes} episodes",
        "status": _rl_state,
    }
    ...
    try:
        ...
        result = {
            ...
            "trade_recommendation": (
                asdict(analysis.trade_recommendation)
...
async def ai_trader_clear_queue():
    ...
    try:
        ...
        trader.opportunity_queue = [
            o for o in trader.opportunity_queue if o.status != "pending"
        ]
        trader._save_state()
...
async def ai_trader_test_paper_trade(symbol: str = "AAPL", quantity: int = 10):
    ...
    try:
        ...
        from ai.ai_watchlist_trader import (TradeOpportunity,
                                            get_ai_watchlist_trader)
        ...
        try:
            ...
            price_resp = requests.get(
                f"http://localhost:9100/api/price/{symbol}", timeout=5
            )
...
async def ai_trader_update_config(data: dict):
    ...
    try:
        ...
        return {"success": True, "updated": updated, "config": trader.config}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def get_pipeline_status():
    ...
    try:
        ...
        return {"success": True, **pipeline.get_status()}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def stop_pipeline():
    ...
    try:
        ...
        pipeline = get_trading_pipeline()
        pipeline.stop()
        return {"success": True, "message": "Trading pipeline stopped"}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def update_pipeline_config(data: dict):
    ...
    try:
        ...
        if (
            data.get("auto_execute")
            and data.get("confirm_auto_execute") == "I_UNDERSTAND_RISKS"
        ):
...
async def run_full_pipeline_cycle():
    ...
    try:
        ...
        return {"success": True, "cycle_complete": True, **results}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def log_trade_to_journal(data: dict):
    ...
    try:
        ...
        return {
            ...
            "trade_id": trade.id,
            "message": (
                "Trade logged successfully" if success else "Failed to log trade"
            ),
        }
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def get_trading_window():
    ...
    try:
        ...
        return {"success": True, **coach.get_trading_window()}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def get_morning_briefing():
    ...
    try:
        ...
        return {"success": True, "briefing": asdict(briefing)}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def critique_trade(trade_data: dict):
    ...
    try:
        ...
        return {"success": True, "critique": asdict(critique)}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def get_coaching_summary():
    ...
    try:
        ...
        coach = get_trading_coach()
        return {"success": True, **coach.get_coaching_summary()}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def get_emotional_patterns():
    ...
    try:
        ...
        coach = get_trading_coach()
        return {"success": True, "patterns": coach.emotional_patterns}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def reset_emotional_patterns():
    ...
    try:
        ...
        coach._save_state()
        return {"success": True, "message": "Emotional patterns reset"}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def stop_premarket_trading():
    ...
    return {"success": True, "message": "Auto-trading stopped", "summary": summary}
        ...
        try:
            ...
            try:
                ...
                if scanner_results.get("setups"):
                    logger.info(
                        f"[PREMARKET] Found {len(scanner_results['setups'])} setups"
                    )
...
def _process_premarket_setup(setup: dict):
    ...
    if _auto_trading_state["paper_mode"]:
        ...
        logger.info(
            f"[PREMARKET] Paper trade: {symbol} PnL=${pnl:.2f} (Total: ${_auto_trading_state['total_pnl']:.2f})"
        )
...
async def get_polygon_daily(symbol: str, days: int = 90):
    ...
    try:
        ...
        bars = polygon.get_daily_bars(
            symbol, from_date=from_date, to_date=to_date, limit=days
        )
...
async def get_backtest_status():
    ...
    return {
        ...
        "data_sources": {"yfinance": True, "polygon": HAS_POLYGON},
        "strategies": ["AI Prediction (LightGBM)", "Pattern Recognition"],
    }
