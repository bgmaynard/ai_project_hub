...
def get_config():
    ...
    if brain:
        return {
            ...
            "TRAIL_PERCENT": brain.get_trailing_stop_percent()
...
class PositionGuardian:
    ...
    def sell_position(self, symbol, qty, reason):
        ...
        try:
            ...
            if bid_price and bid_price > 0:
                ...
                self.log(
                    f"Placing LIMIT sell @ ${fallback_limit:.2f} (fallback, no bid)"
                )
        ...
        for pos in positions:
            ...
            if current_price > self.high_water_marks.get(symbol, 0):
                ...
                if current_price > old_hwm * 1.005:  # Log significant new highs
                    self.log(
                        f"NEW HIGH: {symbol} ${current_price:.2f} (was ${old_hwm:.2f})"
                    )
            ...
            if pnl_from_entry > min_profit and drop_from_high >= trail_pct:
                should_sell = True
                reason = (
                    f"TRAIL STOP: dropped {drop_from_high:.1%} from high ${hwm:.2f}"
                )
            ...
            # This catches orphan positions from crashes, bad trades, etc.
                ...
                should_sell = True
                reason = (
                    f"ZOMBIE KILLER: position down {pnl_from_entry:.1%}, cutting losses"
                )
            ...
            if not should_sell and symbol in self.entry_times:
                ...
                if (
                    position_age_seconds > max_hold and pnl_from_entry < 0.03
                ):  # < 3% gain
                    should_sell = True
            ...
            if should_sell:
                ...
                self.log(
                    f"  Entry: ${entry:.2f} | High: ${hwm:.2f} | Now: ${current_price:.2f}"
                )
    ...
    def startup_scan(self):
        ...
        if violations:
            ...
            for v in violations:
                self.log(
                    f"  {v['symbol']}: ${v['current']:.2f} ({v['pnl_pct']:.1%}) - {v['reason']}"
                )
    ...
    def run(self):
        ...
        self.log(
            f"  3. Zombie killer: {CONFIG['ZOMBIE_LOSS_PERCENT']:.1%} loss = immediate sell"
        )
        self.log(f"  4. Max spread: ${CONFIG['MAX_SPREAD_CENTS']:.2f} - sell if wider")
        self.log(
            f"  5. STALE POSITION: {CONFIG.get('STALE_POSITION_SECONDS', 300)/60:.0f} min not profitable = EXIT"
        )
        self.log(
            f"  6. MAX HOLD: {CONFIG.get('MAX_HOLD_SECONDS', 900)/60:.0f} min max (unless big winner)"
        )
        self.log("=" * 60)
        ...
        while True:
            try:
                ...
                if cycle % 60 == 0:  # Status every 60 seconds
                    ...
                    if long_positions:
                        self.log(
                            f"[{cycle}] Guarding {len(long_positions)} positions | "
                        ...
                        for p in long_positions:
                            ...
                            self.log(
                                f"  {sym}: ${curr:.2f} | Entry: ${entry:.2f} ({pnl:+.1f}%) | "
