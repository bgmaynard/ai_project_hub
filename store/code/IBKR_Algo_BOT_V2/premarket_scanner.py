...
def get_yahoo_movers() -> Dict[str, List]:
    ...
    try:
        ...
        return {'gainers': [], 'losers': [], 'active': [], 'source': 'Yahoo', 'error': str(e)}
...
def add_to_watchlist(symbol: str) -> bool:
    ...
    try:
        r = requests.post(f"http://localhost:9100/api/watchlist/add/{symbol}", timeout=5)
...
def main():
    ...
    if yahoo_gainers:
        print(f"  [OK] Yahoo: {len(yahoo_gainers)} gainers, {len(yahoo_active)} most active")
    else:
    ...
    # Print results
    print_table("SCALP CANDIDATES (Price $1-$20, Gap 5%+, Vol 500K+)", scalp_candidates, 15)

    if scalp_candidates:
        print(f"\n  Score = (Change% × 2) + (Volume in millions)")

    print_table("ALL GAINERS (Top 15)", sorted(unique_gainers, key=lambda x: x.get('change_pct', 0), reverse=True), 15)
