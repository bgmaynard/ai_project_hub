...
class HFTScalperREST:
    ...
    def update_candle_patterns(self, symbol: str):
        ...
        for i, bar in enumerate(bars[-5:]):
            ...
            if body / total_range < 0.3 and lower_ratio > 0.6 and self.hard_resistance_break.get(symbol):
    ...
    def update_vwap(self, symbol: str, price: float, volume: float):
        ...
        if self.vwap_cumulative_vol[symbol] > 0:
            ...
            if is_above and not was_above:
                ...
                logger.info(f"VWAP CROSS UP: {symbol} @ ${price:.2f} > VWAP ${new_vwap:.2f}")
            elif not is_above:
    ...
    def scan_historical_sr(self, symbol: str):
        ...
        if peaks:
            ...
            for c in clusters:
                logger.info(f"HARD RESISTANCE: {symbol} @ ${c['price']:.2f} ({c['touches']} touches)")
        ...
        for level in self.resistance_levels.get(symbol, []):
            if price > level['price'] * 1.005:  # 0.5% above resistance
                if not self.hard_resistance_break[symbol]:
                    ...
                    logger.info(f"DOUBLE TOP BREAK: {symbol} broke ${level['price']:.2f} ({level['touches']} touches) -> MOMENTUM")
                return
    ...
    def check_entry(self, symbol: str):
        ...
        if strength >= 2:  # Need at least 2 confirming signals
            ...
            logger.info(f">>> ENTRY SIGNAL: {symbol} @ ${price:.2f} | Strength={strength} | {signals}")
    ...
    def check_exit(self, symbol: str):
        ...
        if should_exit:
            ...
            if result.get('success'):
                ...
                logger.info(f"<<< SOLD {symbol}: {qty} shares @ ${limit_price:.2f} | P/L: ${(price-entry)*qty:.2f}")
        ...
        try:
            while self.running:
                ...
                if cycle % 20 == 0:
                    ...
                    logger.info(f"[{cycle}] Positions: {pos_str} | Trades: {self.trades_executed} | P/L: ${self.total_pnl:.2f}")
