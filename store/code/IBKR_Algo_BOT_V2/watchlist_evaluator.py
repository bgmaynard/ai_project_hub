...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('watchlist_evaluator.log')
    ]
)
...
class WatchlistEvaluator:
    ...
    def _load_results(self):
        ...
        if EVAL_RESULTS_FILE.exists():
            try:
                ...
                logger.info(f"Loaded {len(self.evaluation_results)} existing evaluations")
            except:
                pass
    ...
    def _save_results(self):
        ...
        EVAL_RESULTS_FILE.write_text(json.dumps(self.evaluation_results, indent=2, default=str))
    ...
    def evaluate_symbol(self, symbol: str) -> Dict:
        ...
        logger.info(f"  AI Prediction: {prediction.get('signal', 'N/A')} ({prediction.get('confidence', 0):.1%})")
    ...
    def _generate_prediction(self, symbol: str) -> Dict:
        ...
        try:
            if self.predictor:
                ...
                if pred:
                    result.update(pred)
                    logger.info(f"  Prediction: {pred.get('signal')} @ {pred.get('confidence', 0):.1%}")
            else:
    ...
    def _simple_momentum_prediction(self, symbol: str) -> Dict:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if change_pct > 5:
                    result = {'signal': 'STRONG_BULLISH', 'confidence': 0.8, 'prob_up': 0.75}
                elif change_pct > 2:
                    result = {'signal': 'BULLISH', 'confidence': 0.7, 'prob_up': 0.65}
                elif change_pct < -5:
                    result = {'signal': 'STRONG_BEARISH', 'confidence': 0.8, 'prob_up': 0.25}
                elif change_pct < -2:
                    result = {'signal': 'BEARISH', 'confidence': 0.7, 'prob_up': 0.35}
                else:
                    result = {'signal': 'NEUTRAL', 'confidence': 0.5, 'prob_up': 0.5}

                logger.info(f"  Momentum-based prediction: {result['signal']} (change: {change_pct:.1f}%)")
        except Exception as e:
    ...
    def _simple_backtest(self, symbol: str, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            logger.info(f"  Backtest: {result['trades']} trades, {result['win_rate']:.1%} win rate, {result['total_return']:.1%} return")
        except Exception as e:
    ...
    def _api_backtest(self, symbol: str) -> Dict:
        ...
        try:
            ...
            r = requests.get(f"{API_BASE}/api/alpaca/bars/{symbol}?timeframe=1Day&limit=30", timeout=10)
            if r.status_code == 200:
                bars = r.json()
                if bars and len(bars) > 5:
                    ...
                    if closes:
                        ...
                        if returns:
                            ...
                            result['win_rate'] = winning / len(returns) if returns else 0.5
                            result['total_return'] = sum(returns)
                            result['trades'] = len(returns)
                            logger.info(f"  API backtest: {result['trades']} periods, {result['win_rate']:.1%} win rate")
        except Exception as e:
    ...
    def _update_watchlist_prediction(self, symbol: str, result: Dict):
        ...
        try:
            ...
            logger.info(f"  Updated {symbol}: {signal} @ {confidence:.1%} (score: {score})")
        except Exception as e:
        ...
        while self.running:
            try:
                ...
                if cycle % 6 == 0:  # Status every minute
                    logger.info(f"[{datetime.now().strftime('%H:%M:%S')}] Monitoring {len(self.known_symbols)} symbols | "
