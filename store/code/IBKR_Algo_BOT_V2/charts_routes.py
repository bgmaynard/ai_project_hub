...
async def get_chart_ohlc(
    ...
    try:
        ...
        if timespan == "day":
            ...
            raw_bars = polygon.get_minute_bars(
                symbol, from_date, to_date, multiplier=multiplier
            )
...
async def get_trade_markers(
    symbol: str,
    timeframe: str = Query(
        "5m", description="Timeframe to align markers: 1m, 5m, 15m, 1h, 1d"
    ),
    ...
    if from_date:
        ...
        trades = [
            t
            for t in trades
            if datetime.fromisoformat(t.get("entry_time", "2000-01-01")) >= from_dt
        ]
    ...
    if to_date:
        ...
        trades = [
            t
            for t in trades
            if datetime.fromisoformat(t.get("entry_time", "2099-12-31")) < to_dt
        ]
    ...
    if summary["total_trades"] > 0:
        summary["win_rate"] = round(
            summary["winners"] / summary["total_trades"] * 100, 1
        )
    else:
        summary["win_rate"] = 0
    ...
    return {"success": True, "symbol": symbol, "markers": markers, "summary": summary}
...
async def get_indicators(
    ...
    indicators: str = Query(
        "sma20,ema9,vwap",
        description="Comma-separated: sma20,ema9,ema20,vwap,rsi14,macd",
    ),
    ...
    try:
        ...
        if "timestamp" in df.columns:
            df["time"] = df["timestamp"].apply(
                lambda x: int(x / 1000) if x > 1e12 else int(x)
            )
        else:
...
async def get_equity_curve(
    ...
    curve = [
        {
            "time": int(datetime.now().timestamp()) - 86400 * 30,
            "equity": initial_capital,
        }
    ]
    ...
    for trade in trades:
        if trade.get("status") == "closed" and trade.get("exit_time"):
            ...
            curve.append({"time": exit_time, "equity": round(equity, 2)})
...
async def get_backtest_results(
    ...
    # Calculate summary
    closed_trades = [
        t for t in trades if t.get("status") == "closed" or t.get("exit_time")
    ]
    ...
    return {
        ...
        "report": {
            ...
            "total_pnl": round(total_pnl, 2),
            "avg_win": (
                round(sum(t.get("pnl", 0) for t in winners) / len(winners), 2)
                if winners
                else 0
            ),
            "avg_loss": (
                round(sum(t.get("pnl", 0) for t in losers) / len(losers), 2)
                if losers
                else 0
            ),
...
try:
    from ai.technical_signals import SignalEvent, get_signal_analyzer
...
async def get_technical_signals(
    ...
    try:
        ...
        return {
            ...
            "recommendation": {"should_enter": should_enter, "reason": enter_reason},
        }
...
async def get_multi_timeframe_alignment(
    symbol: str, days: int = Query(5, description="Days of history")
):
    """
    ...
    try:
        ...
        return {"success": True, "symbol": symbol, "alignment": alignment}
